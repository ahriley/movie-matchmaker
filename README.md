...
where _x_ and _y_ are different users, _I<sub>xy<\sub> contains the movies that users x and y both rated, rx,i and ry,i  are the ratings given to movie i by user x and y, respectively, and rx, ry is the mean rating over all movies in set Ixy  given by user x, y.
